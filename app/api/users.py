...
async def promote_user(
    ...
    try:
        ...
        await db.commit()
        await db.refresh(promoted_user)
...
async def deactivate_user_endpoint(
    ...
    try:
        deactivated_user = await deactivate_user(
            db=db,
            user_id=user_id,
            current_user_id=current_admin.id,
            tenant_id=current_admin.tenant_id,
        ...
        await create_audit_log(
            ...
            tenant_id=current_admin.tenant_id,
            actor_user_id=current_admin.id,
            target_user_id=deactivated_user.id,
...
async def reactivate_user_endpoint(
    ...
    try:
        reactivated_user = await reactivate_user(
            db=db,
            user_id=user_id,
            tenant_id=current_admin.tenant_id,
        )
        ...
        await create_audit_log(
            ...
            tenant_id=current_admin.tenant_id,
            actor_user_id=current_admin.id,
            target_user_id=reactivated_user.id,
            action=AuditAction.REACTIVATE_USER,
        )

        await db.commit()
        await db.refresh(reactivated_user)
